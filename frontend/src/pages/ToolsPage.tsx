    ...
    return (
        <div className="modal-overlay" onClick={onClose}>
            <div className="modal-sheet" onClick={e => e.stopPropagation()}>
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
                        {calc.fields.map(f => (
                            <div key={f.key} className="form-group">
                                ...
                                {f.hint && <div style={{ fontSize: '0.7rem', color: '#475569', marginTop: 2 }}>{f.hint}</div>}
                            </div>
                        ))}
                ...
                {/* Copiar resultado */}
                {result && <CopyButton label={`${calc.name}: ${result.value} ${result.unit ?? ''} — ${result.interpretation ?? ''}`} />}
            </div>
        </div>
    );
}
    ...
    return (
        <div className="modal-overlay" onClick={onClose}>
            <div className="modal-sheet" onClick={e => e.stopPropagation()}>
                ...
                {hasAll && result && (
                    <CopyButton label={`${scale.name}: ${total} pts — ${result.text}`} />
                )}
            </div>
        </div>
    );
...
function CopyButton({ label }: { label: string }) {
    ...
    return (
        <div style={{ padding: '0 20px 20px' }}>
            <button onClick={() => {
                navigator.clipboard.writeText(`[FisioSim Pro] ${label}`).then(() => {
                    setCopied(true); setTimeout(() => setCopied(false), 2000);
                }).catch(() => { });
            }} style={{
                display: 'flex', alignItems: 'center', gap: 8, width: '100%',
                padding: '10px 16px', borderRadius: 10, cursor: 'pointer',
                background: copied ? 'rgba(74,222,128,0.1)' : 'rgba(56,189,248,0.08)',
                border: `1px solid ${copied ? 'rgba(74,222,128,0.3)' : 'rgba(56,189,248,0.2)'}`,
...
// ─── CARD DE FERRAMENTA ──────────────────────────────────────────
function ToolCard({ id, icon, name, description, category, onOpen, isFav, onToggleFav }: {
    id: string; icon: string; name: string; description: string; category: string;
    onOpen: () => void; isFav: boolean; onToggleFav: (id: string) => void;
}) {
...
export default function ToolsPage() {
    ...
    const [favorites, setFavorites] = useState<Set<string>>(() => {
        try { return new Set(JSON.parse(localStorage.getItem('fisio_favs') ?? '[]')); }
